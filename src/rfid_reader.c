...
esp_err_t rc522_read_card(rfid_result_t *result)
{
    ...
    result->is_authorized = is_authorized_card(result->uid, &holder);
    if (holder) {
        strncpy(result->holder_name, holder, sizeof(result->holder_name) - 1);
    }

    ESP_LOGI(TAG, "Card UID: %s | Authorized: %s",
             result->uid,
             result->is_authorized ? "YES" : "NO");
